...
│                       decor.rs (WaypointKind, Facing, WallDecor, PlantKind, PodDecor; FurnitureDef +
│                         furniture_def(kind) + desk_furniture_def() — the single source of truth for
│                         every furniture FOOTPRINT [blocked ground area], occupies_pos, dwell, ApproachSides;
│                         + the non-waypoint footprint consts [MEETING_SOFA/TABLE/PANTRY_TABLE/CHAIR/LAMP/
│                         SIDE_TABLE/PLANT_FOOTPRINT] and DESK_APPROACH/desk_walk_anchor),
...
- "Where does a furniture's footprint / approach side / dwell come from?" → **`core::layout::decor`** is the single source of truth. `FurnitureDef { footprint, occupies_pos, dwell, approach: ApproachSides }` + `furniture_def(WaypointKind)` (table) and `desk_furniture_def()` (the home desk — same `FurnitureDef` type, NOT a `WaypointKind` row, since it's owned per-agent). Non-waypoint static furniture (meeting sofa/table, pantry table/stool, lamp, side table, plant) declare footprints as co-located consts (`*_FOOTPRINT`); walls use `WALL_THICK_H/V`. **The footprint is the BLOCKED ground area** (z=0 projection) — `mask::build_walkable_mask` stamps it (no inline literals), `approach::{stand_point,walk_target}` resolve the stand/approach cell off it, and the tui z-sort baseline derives from its front edge. **Three load-bearing distinctions:** (1) footprint (blocked ground) ≠ visual sprite (may overhang — `PLANT_FOOTPRINT` 6×6 vs taller `PlantKind::size`; vertical wall footprint 1px vs 3px drawn); (2) `occupies_pos` = the agent sits/stands ON the cell (couch/meeting) vs approaches it from a side (appliances); (3) **placement overlap** (furniture buried in a wall = bug, see `freestanding_decor_does_not_overlap_room_walls`) ≠ **occlusion overlap** (glass-over-sofa, sofa-over-sitter = physically correct, resolved by the tui z-sort/back-cap, NOT a placement error). `ApproachSides` (canonical facing-South, rotated by `allows`) is the editable entry-side knob — flip one bool to drop a side. The unifier is "one source per obstacle type + one shared z-sort," not "one struct."
