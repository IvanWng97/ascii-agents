...
│                         furniture's FOOTPRINT [blocked ground] AND visual [sprite size], occupies_pos, dwell,
│                         ApproachSides; + non-enum footprint consts [MEETING_SOFA/TABLE/PANTRY_TABLE/CHAIR/
│                         LAMP/SIDE_TABLE/PLANT_FOOTPRINT] and DESK_APPROACH/desk_walk_anchor),
...
- "Where does a furniture's footprint / visual size / approach side / dwell come from?" → **`core::layout::decor`** is the single source of truth, and there is now exactly ONE geometry table. `FurnitureDef { footprint, visual, occupies_pos, dwell, approach: ApproachSides }` + `furniture_def(Furniture)` — keyed by the unified `Furniture` geometry enum — plus `desk_furniture_def()` (the home desk: same `FurnitureDef` type, NOT a `Furniture`/waypoint row, since it's owned per-agent). **The role enums carry NO dimensions**: `WaypointKind` (wander destination), `PodDecor` (aisle filler), `PlantKind`, `WallDecor` each expose `.furniture()` mapping onto one `Furniture` row, so overlapping items (a phone booth that is both a waypoint and pod decor, a plant that is both free-standing and pod decor, a whiteboard that is both wall and pod decor) have their shape defined exactly once. Non-enum singleton furniture (meeting sofa/table, pantry table/stool, lamp, side table) still declare ground footprints as co-located `*_FOOTPRINT` consts (single-source, referenced by the table where an enum kind needs them — e.g. `PLANT_FOOTPRINT`); walls use `WALL_THICK_H/V`. **`footprint` is the BLOCKED ground area** (z=0 projection) — `mask::build_walkable_mask` stamps it (no inline literals); **`visual` is the sprite size** — render centering + the z-sort south row (`center_pin_south_offset`) derive from it. **Three load-bearing distinctions:** (1) `footprint` (blocked ground) ≠ `visual` (sprite, may overhang — `Furniture::PlantTall` footprint 6×6 vs visual 6×10, the split that fixed the canopy-blocks-aisle bug; vertical wall footprint 1px vs 3px drawn); (2) `occupies_pos` = the agent sits/stands ON the cell (couch/meeting) vs approaches it from a side (appliances); (3) **placement overlap** (furniture buried in a wall = bug, see `freestanding_decor_does_not_overlap_room_walls`) ≠ **occlusion overlap** (glass-over-sofa, sofa-over-sitter = physically correct, resolved by the tui z-sort/back-cap, NOT a placement error). `ApproachSides` (canonical facing-South, rotated by `allows`) is the editable entry-side knob — flip one bool to drop a side. The unifier is "one geometry table (footprint + visual) + one shared z-sort," not a second `size()` per role enum.
