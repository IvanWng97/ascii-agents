...
- "How does config persistence work?" → `config.rs` defines `AppConfig` (theme as `Option<String>`), `config_path()` (XDG-aware: `$XDG_CONFIG_HOME/ascii-agents/config.toml` or `~/.config/ascii-agents/config.toml`), `load()` (never crashes — logs warning on malformed TOML), `save()` (advisory-locked atomic tmp+rename), `resolve_theme()` (CLI > config > default). Theme saved on `[t]` picker Enter confirm in `tui/mod.rs`. CLI `--theme` is `Option<String>` so config values aren't shadowed by clap defaults. Desk capacity is auto-computed from terminal size, not persisted.
