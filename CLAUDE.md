...
│       ├── motion.rs       per-agent walk-timing state: MotionState (entry/exit/snap_back/wander_*/walk_path fields;
│       │                   exit is a 3-tuple carrying the from-Point; walk_path = frozen per-leg A* polyline via WalkPathSnapshot),
...
│       │                   → drives t_x1000 per-frame via physics::walk_progress. The leg's A* polyline
│       │                   *shape* is also frozen once per leg (walk_path, re-snapshotted only when (from,to)
│       │                   changes) so per-frame overlay churn can't reroute an in-flight walker (no flash).
